...
name = "dolar.csv"  # name of csv used


# class responsible for trading symulation
class trade:
    __name = ""
    __money = 0
    ...
    def trading(self):
        ...
        if bought > 0:
            ...
            profit = (self.__money - starting_money) / starting_money * 100
            ...
            print("Profit equals: " + str(profit) + "%.")


# class responsible for taking data form csv file
class data:
    __name = ""
    ...
    def getData(self):
        ...
        days_reversed = []
        for i in range(0, N):
            close_data.append(float(data.iloc[i, 2]))
...
# class responsible for counting all indicators
class Macd:
    __macd = []
    __signal = []
    __date = []
    __buy_and_sell_signals = []
    ...
    def buySell(self):  # sygnal = 0 means do nothing, = 1 means buy and = -1 means sell
        self.__buy_and_sell_signals.clear()
        self.__buy_and_sell_signals.append(int(0))
        for var in range(1, len(self.__macd) - 1):
            if self.__macd[var - 1] > self.__signal[var - 1] and self.__macd[var] <= self.__signal[var] and self.__macd[
                var] >= 0:  # checking way of crossing macd and signal
                if self.__macd[var - 1] >= self.__macd[var]:  # checking trend
                    self.__buy_and_sell_signals.append(int(-1))
            elif self.__macd[var - 1] < self.__signal[var - 1] and self.__macd[var] >= self.__signal[var] and \
                    self.__macd[var] <= 0:  # checking way of crossing macd and signal
                if self.__macd[var - 1] <= self.__macd[var]:  # checking trend
                    self.__buy_and_sell_signals.append(int(1))
            else:
                self.__buy_and_sell_signals.append(int(0))
    ...
    def getBuySell(self):
        return self.__buy_and_sell_signals


# class responsible for creating plots form given csv file name
class Plot:
    def __init__(self, name):
...
def main():
    Plot(name)  # creates all plots
    trade(name, 1000)  # simulating trading for 1000 days with starting money = 1000 PLN
